...
const TYPE_COLOR    = { ATRIAL: '#4BBFBF', ENLEVEMENT: '#5B8DB8', TRANSPORTEUR: '#E8A838' };
const TYPE_LABEL    = { ATRIAL: 'Atrial', ENLEVEMENT: 'Enlèvement', TRANSPORTEUR: 'Transporteur' };
...
async function checkAuth() {
  try {
    ...
    if (!['ADV', 'ADMIN'].includes(user.role)) { window.location.href = '/'; return; }
...
function renderDashboard() {
  ...
  // Affaires = stops sans ordre défini (ordre = 99 = non planifiés via webhook)
  ...
  if (!preview.length) {
    ...
    previewEl.innerHTML = preview.map(s => `
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--line-soft)">
        <div style="width:26px;height:26px;border-radius:50%;background:${TYPE_COLOR[s.type]};color:#fff;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0">${s.ordre ?? '?'}</div>
        ...
        <span class="pill ${STATUS_CLASS[s.statut]}">${STATUS_LABEL[s.statut]}</span>
      </div>
    `).join('');
  }
...
function affaireRowHTML(s, compact) {
  ...
  <div class="affaire-row">
    <div style="width:10px;height:10px;border-radius:50%;background:${TYPE_COLOR[s.type]};flex-shrink:0;margin-top:4px"></div>
    <div class="affaire-info">
      ...
      <div class="a-meta">
        ...
        <span class="type-badge ${s.type.toLowerCase()}">${TYPE_LABEL[s.type]}</span>
      </div>
    </div>
...
async function confirmAssign() {
  ...
  try {
    ...
    const maxOrdre = allStops.filter(s => s.ordre !== 99).reduce((m, s) => Math.max(m, s.ordre || 0), 0);
...
function renderTournee() {
  const filtered = activeFilter === 'ALL'
    ? allStops
    : allStops.filter(s => s.type === activeFilter);
  ...
  tbody.innerHTML = filtered.map(s => `
    <tr>
      ...
      <td><span class="type-badge ${s.type.toLowerCase()}">${TYPE_LABEL[s.type]}</span></td>
      <td><span class="pill ${STATUS_CLASS[s.statut]}">${STATUS_LABEL[s.statut]}</span></td>
      <td>
        ${s.statut !== 'LIVRE' ? `
...
async function createStop() {
  ...
  try {
    const res = await fetch('/api/stops', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ societe, adresse, telephone: telephone || null, numero_affaire: affaire || null, type }),
    });
    if (!res.ok) {
...
function renderAdvMap() {
  ...
  if (window._advMarkers) window._advMarkers.forEach(m => m.setMap(null));
  window._advMarkers = [];
  ...
  located.forEach((s, i) => {
    ...
    const m = new google.maps.Marker({
      position: pos, map: advMap,
      title: s.societe,
      label: { text: String(s.ordre ?? i + 1), color: '#fff', fontWeight: '700', fontSize: '11px' },
  ...
  if (located.length > 1) advMap.fitBounds(bounds, { padding: 20 });
}
